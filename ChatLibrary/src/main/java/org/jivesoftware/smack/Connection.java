 ...
 * Once stopped you can use {} to manually connect to the
 * server.
 *
 * @author Matt Tucker
...
public abstract class Connection {
    ...
    public static final String CODE_AUTH = "CODE-AUTH";
    ...
    /**
     * sending packet listener(hieuhd3)
     * */
    protected final List<SendingPacketListener> sendListListeners = new ArrayList<SendingPacketListener>();
     ...
     *
     * @throws XMPPException         if an error occurs or anonymous logins are not supported by
     *                               the server.
     * @throws IllegalStateException if not connected to the server, or already logged in to the
     *                               serrver.
     */
//    public abstract void loginAnonymously() throws XMPPException;
    ...
    /**
     * Returns the roster for the user logged into the server. If the user has
     * not yet logged into the server (or if the user is logged in anonymously),
...
//        Log.i(TAG, "[VIETTEL] addPacketListener = " + packetListener.getClass().getSimpleName());
     ...
     * hieuhd3
     * */
    public void addPacketSendingListener(SendingPacketListener packetListener) {
        if (packetListener == null) {
            throw new NullPointerException("2x.Packet listener is null.");
        }
        sendListListeners.add(packetListener);
    }

    /**
     * hieuhd3
     * */
    public void deletePacketSendingListener(SendingPacketListener packetListener) {
        sendListListeners.remove(packetListener);
    }
    ...
    protected void firePacketSendingListeners(Packet packet) {
        ...
        final List<SendingPacketListener> pListeners = sendListListeners;
        for (SendingPacketListener listener : pListeners) {
            listener.fireSendingPacket(packet);
        }
    }
    ...
    protected void initDebugger() {
        ...
        if (config.isDebuggerEnabled()) {
            ...
            Log.i(TAG, "[perform] init debugger take " + (System.currentTimeMillis() - beginTime) + "ms");
        } else {
            Log.i(TAG, "debugger not enabled");
        }
    }
    ...
    protected static class ListenerWrapper {
         ...
         * @param packetFilter   the associated filter or null if it listen for all
         *                       packets.
         */
    ...
    protected static class InterceptorWrapper {
         ...
         * @param packetFilter      the associated filter or null if it intercepts all
         *                          packets.
         */
