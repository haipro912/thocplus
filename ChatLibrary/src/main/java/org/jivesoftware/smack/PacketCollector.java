...
public class PacketCollector {
    ...
    public synchronized Packet nextResult(long timeout) {
        ...
        if (resultQueue.isEmpty()) {
            ...
            try {
                ...
                // or a packet is available to return.
                while (resultQueue.isEmpty()) {
                    if (waitTime <= 0) {
                        break;
                    }
    ...
    /**
     * goi ham nay khi connection notify error de tranh truong hop conneciton shutdown
     * roi nhung van bi wait --> den khi timeout packet moi het
     */
    protected synchronized void selfNotify() {
        Log.i(TAG, "selfNotify");
        try {
            notifyAll();
        } catch (Exception e) {
