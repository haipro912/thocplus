...
public class SASLAuthentication implements UserAuthentication {
    ...
    private static final String TAG = "SASLAuthentication";
    ...
    static {
        ...
        registerSASLMechanism("CODE-AUTH", SASLCodeAuthMechanism.class);
        registerSASLMechanism("TOKEN-AUTH", SASLTokenAuthMechanism.class);
        registerSASLMechanism("PASSWORD-AUTH", SASLPasswordAuthMechanism.class);

        // supportSASLMechanism("GSSAPI",0);
        supportSASLMechanism("CODE-AUTH", 0);
        supportSASLMechanism("TOKEN-AUTH", 1);
        supportSASLMechanism("PASSWORD-AUTH", 2);
        supportSASLMechanism("DIGEST-MD5", 3);
        // supportSASLMechanism("CRAM-MD5",2);
        supportSASLMechanism("PLAIN", 4);
        supportSASLMechanism("ANONYMOUS", 5);
    }
     ...
     *
     * @param name   common name of the SASL mechanism. E.g.: PLAIN, DIGEST-MD5 or
     *               KERBEROS_V4.
     ...
     * @param name  common name of the SASL mechanism. E.g.: PLAIN, DIGEST-MD5 or
     *              KERBEROS_V4.
     * @param index preference position amongst all the implemented SASL
    ...
    public String authenticate(String username, String password, String resource,
        ...
        if (selectedMechanism != null) {
            ...
            try {
                ...
                currentMechanism = (SASLMechanism) constructor
                        .newInstance(this);
                ...
                currentMechanism.authenticate(username,
                ...
                if (saslNegotiated) {
                    ...
                    return new NonSASLAuthentication(connection).authenticate(
                            username, password, resource, revision, countryCode);
                }
            ...
            return new NonSASLAuthentication(connection).authenticate(username,
                    password, resource, revision, countryCode);
        }
    }
    ...
    private Bind bindResourceAndEstablish(String resource)
            throws XMPPException {
        // Wait until server sends response containing the <bind> element
        synchronized (this) {
            if (!resourceBinded) {
     ...
     *
     * @param mechanisms collection of strings with the available SASL mechanism
     ...
     */
    void authenticationFailed(String condition, int retries) {
        synchronized (this) {
            saslFailed = true;
    ...
    void authenticationFailed(int lockedTime) {
        synchronized (this) {
            saslFailed = true;
    ...
    void authenticationFailed(SASLMechanism.Failure failureRespone) {
        synchronized (this) {
            saslFailed = true;
            this.lockedTime = failureRespone.getLockedTime();
            this.retries = failureRespone.getRetries();
            errorCondition = failureRespone.getCondition();
            // Wake up the thread that is waiting in the #authenticate method
            notify();
        }
    }
