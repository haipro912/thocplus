 ...
 * $RCSfile$
 * $Revision: 10846 $
 * $Date: 2008-10-24 07:17:50 +0200 (ven. 24 oct. 2008) $
 * <p/>
...
public final class SmackConfiguration {
    private static final String TAG = SmackConfiguration.class.getSimpleName();
    private static final String SMACK_VERSION = "3.1.0";

    private static int packetReplyTimeout = 20000;//20s
    private static int packetRosterReplyTimeout = 20000;//20s
    ...
    static {
        try {
            ...
            for (ClassLoader classLoader : classLoaders) {
                Enumeration configEnum = classLoader
    ...
    // add toan vk2
    public static int getPacketRosterReplyTimeout() {
        if (packetRosterReplyTimeout <= 0) {
            packetRosterReplyTimeout = 20000;
        }
        return packetRosterReplyTimeout;
    }

    public static void setPacketRosterReplyTimeout(int timeout) {
        if (timeout <= 0) {
            throw new IllegalArgumentException();
        }
        packetRosterReplyTimeout = timeout;
    }
     ...
     *
     * @param interval the milliseconds to wait between keep-alive requests, or -1 if
    ...
    public static void setPingInterval(int pingInterval) {
        SmackConfiguration.pingInterval = pingInterval;
    }
